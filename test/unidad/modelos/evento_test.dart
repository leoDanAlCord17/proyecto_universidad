...
void main() {
  ...
  group('Evento.aJson', () {
    ...
    test('_formatearFecha produce yyyy-MM-dd con padding correcto', () {
      final evento = Evento.desdeJson(const <String, dynamic>{
        'id': 'x', 'titulo': 'x', 'modo_registro': 'auto',
        'estatus': 'programado',
    ...
    test('fechas null producen null en el json', () {
      final evento = Evento(
        id: 'x', titulo: 'x', modoRegistro: 'auto', estatus: 'programado',
        creadoEn: DateTime.utc(2025), actualizadoEn: DateTime.utc(2025),
        permiteQrEvento: true, permiteQrUsuario: true, permiteManualAdmin: true,
        requiereCicloCompleto: false, permiteSalidaAnticipada: false,
        marcarAusentesAuto: false, permiteForaneos: false,
      );
      final json = evento.aJson();
  ...
  group('EventoConGrupos._construirGrupos', () {
    Map<String, dynamic> baseJson({
      ...
      List<Map<String, dynamic>> grupos = const [],
        ...
        {
          'id': 'ev-1', 'titulo': 'T', 'modo_registro': 'auto',
          'estatus': 'programado', 'alcance': alcance,
          'creado_en': '2025-01-01T00:00:00Z',
          'actualizado_en': '2025-01-01T00:00:00Z',
    ...
    test('grupo con solo tag principal se construye correctamente', () {
      final ecg = EventoConGrupos.desdeJson(baseJson(
        alcance: 'dirigido',
        grupos: [
          {'grupo_index': 0, 'tag_id': 'tp-1', 'tags': {'tipo': 'principal', 'nombre': 'Ing'}},
        ],
      ),);
    ...
    test('grupo con principal y secundarios se construye correctamente', () {
      final ecg = EventoConGrupos.desdeJson(baseJson(
        ...
        grupos: [
          {'grupo_index': 0, 'tag_id': 'tp-1', 'tags': {'tipo': 'principal',  'nombre': 'Ing'}},
          {'grupo_index': 0, 'tag_id': 'ts-1', 'tags': {'tipo': 'secundario', 'nombre': 'Sis'}},
          {'grupo_index': 0, 'tag_id': 'ts-2', 'tags': {'tipo': 'secundario', 'nombre': 'Redes'}},
        ],
      ),);
    ...
    test('dos grupos independientes se construyen por separado', () {
      final ecg = EventoConGrupos.desdeJson(baseJson(
        alcance: 'dirigido',
        grupos: [
          {'grupo_index': 0, 'tag_id': 'tp-1', 'tags': {'tipo': 'principal', 'nombre': 'A'}},
          {'grupo_index': 1, 'tag_id': 'tp-2', 'tags': {'tipo': 'principal', 'nombre': 'B'}},
        ],
      ),);
    ...
    test('nombresParaBusqueda agrega todos los nombres de todos los grupos', () {
      final ecg = EventoConGrupos.desdeJson(baseJson(
        alcance: 'dirigido',
        grupos: [
          {'grupo_index': 0, 'tag_id': 'tp-1', 'tags': {'tipo': 'principal',  'nombre': 'Ingeniería'}},
          {'grupo_index': 0, 'tag_id': 'ts-1', 'tags': {'tipo': 'secundario', 'nombre': 'Sistemas'}},
        ],
      ),);
