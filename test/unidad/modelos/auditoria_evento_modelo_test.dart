...
RegistroAuditoria _registroConEstatus(String estatus, {bool esForaneo = false}) =>
    RegistroAuditoria(
      id:           'reg-$estatus',
      nombre:       'Test',
      iniciales:    'T',
...
void main() {
  ...
  group('EventoParaAuditoria.etiquetaEstatus', () {
    test('en_curso → "En curso"',    () => expect(EventoParaAuditoria.desdeJson(_jsonEvento(estatus: 'en_curso')).etiquetaEstatus,    'En curso'));
    test('programado → "Programado"',() => expect(EventoParaAuditoria.desdeJson(_jsonEvento(estatus: 'programado')).etiquetaEstatus,  'Programado'));
    test('finalizado → "Finalizado"',() => expect(EventoParaAuditoria.desdeJson(_jsonEvento(estatus: 'finalizado')).etiquetaEstatus,  'Finalizado'));
    test('cancelado → "Cancelado"',  () => expect(EventoParaAuditoria.desdeJson(_jsonEvento(estatus: 'cancelado')).etiquetaEstatus,   'Cancelado'));
    test('borrador → "Borrador"',    () => expect(EventoParaAuditoria.desdeJson(_jsonEvento(estatus: 'borrador')).etiquetaEstatus,    'Borrador'));
    test('desconocido → raw',        () => expect(EventoParaAuditoria.desdeJson(_jsonEvento(estatus: 'foo')).etiquetaEstatus,         'foo'));
  });
  ...
  group('RegistroAuditoria.desdeJson — usuario registrado', () {
    ...
    test('estatus se mapea correctamente', () {
      final r = RegistroAuditoria.desdeJson(_jsonRegistroUsuario(estatus: 'completado'));
  ...
  group('RegistroAuditoria.haEntrado', () {
    test('true para "presente"',         () => expect(_registroConEstatus('presente').haEntrado,         isTrue));
    test('true para "completado"',       () => expect(_registroConEstatus('completado').haEntrado,       isTrue));
    test('true para "salio_anticipado"', () => expect(_registroConEstatus('salio_anticipado').haEntrado, isTrue));
    test('false para "ausente"',         () => expect(_registroConEstatus('ausente').haEntrado,          isFalse));
    test('false para "esperado"',        () => expect(_registroConEstatus('esperado').haEntrado,         isFalse));
    test('false para "anulado"',         () => expect(_registroConEstatus('anulado').haEntrado,          isFalse));
  });

  group('RegistroAuditoria.etiquetaEstatus', () {
    test('presente → "Presente"',               () => expect(_registroConEstatus('presente').etiquetaEstatus,         'Presente'));
    test('completado → "Completado"',           () => expect(_registroConEstatus('completado').etiquetaEstatus,       'Completado'));
    test('ausente → "Ausente"',                 () => expect(_registroConEstatus('ausente').etiquetaEstatus,          'Ausente'));
    test('esperado → "Esperado"',               () => expect(_registroConEstatus('esperado').etiquetaEstatus,         'Esperado'));
    test('salio_anticipado → "Anticipado"',     () => expect(_registroConEstatus('salio_anticipado').etiquetaEstatus, 'Anticipado'));
    test('anulado → "Anulado"',                 () => expect(_registroConEstatus('anulado').etiquetaEstatus,          'Anulado'));
    test('desconocido → raw',                   () => expect(_registroConEstatus('foo').etiquetaEstatus,              'foo'));
  });
    ...
    test('timeline agrupa entradas por hora', () {
      final registros = [
        const RegistroAuditoria(
          id: '1', nombre: 'A', iniciales: 'A',
          estatus: 'presente', esForaneo: false,
          horaEntrada: '', horaSalida: '',
          horaEntradaHora: 10,
        ),
        const RegistroAuditoria(
          id: '2', nombre: 'B', iniciales: 'B',
          estatus: 'presente', esForaneo: false,
          horaEntrada: '', horaSalida: '',
          horaEntradaHora: 10,
        ),
        const RegistroAuditoria(
          id: '3', nombre: 'C', iniciales: 'C',
          estatus: 'presente', esForaneo: false,
          horaEntrada: '', horaSalida: '',
          horaEntradaHora: 14,
        ),
