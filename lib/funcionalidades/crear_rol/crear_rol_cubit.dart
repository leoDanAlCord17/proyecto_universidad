...
class CrearRolCubit extends Cubit<CrearRolEstado> {
  ...
  Future<void> guardar({required String nombre, required String descripcion}) async {
    final e = state;
    if (e is! CrearRolCargado) return;
    emit(e.copiarCon(estaGuardando: true));
    try {
      final editando = e.rolId != null;
      final datos    = {'nombre': nombre, 'descripcion': descripcion, 'estatus': true};
      final rolId    = editando
          ? e.rolId!
